...
module.exports = class Generator {
    ...
    _generate4() {
        const fs = require('fs');
        const list = fs.readdirSync(`${process.cwd()}/${this.src}`, {withFileTypes: true}).filter(f => !f.isDirectory()).map(f => `${process.cwd()}/${this.src}/${f.name}`);
        let datapacks = [];
        list.forEach(f => {
            try {
                const c = require(f);
        ...
        fs.rmdirSync(`${process.cwd()}/${this.dist}`, {recursive: true});
        fs.mkdirSync(`${process.cwd()}/${this.dist}`);
        datapacks.forEach(datapack => {
            datapack.packs.forEach(pack => {console.log(pack);
                fs.mkdirSync(`${process.cwd()}/${this.dist}/${pack.name}`);
                fs.mkdirSync(`${process.cwd()}/${this.dist}/${pack.name}/data`);
                fs.writeFileSync(`${process.cwd()}/${this.dist}/${pack.name}/pack.mcmeta`, JSON.stringify({pack_format: this.packFormat, description: pack.description}, 0, 4));
            });
        });
    }
};
